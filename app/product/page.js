    ...
    (async () => {
      try {
        const { data, error } = await supabase
          .from("products")
          .select("id,nama,brand_slug,kategori,price,image_url,image_urls,deskripsi,is_active,status,date_created")
          .eq("status", "published")
          .eq("is_active", true)
      ...
      <main className="w-full min-h-[100dvh] bg-white md:max-w-[430px] md:shadow md:border flex flex-col overflow-y-auto pb-[80px]">
        ...
        <section className="px-4 pb-2">
          <div className="flex items-center justify-between mb-2">
            ...
            <Link href="/brand" className="text-[#D6336C] text-sm">Lihat Semua</Link>
          </div>
          ...
          {brands === null ? (
            ...
            <div className="flex gap-3 overflow-x-auto no-scrollbar snap-x snap-mandatory pb-2">
              {brands
                .filter((b) => b?.slug)
                .map((b, i) => {
                  const hasLogo = typeof b?.logoSrc === "string" && b.logoSrc.trim().length > 0;
                  const logo = hasLogo ? b.logoSrc : "/brand/brand-placeholder.svg";
                  return (
        ...
        <section className="px-4 pb-2">
          ...
          {cats === null ? (
            ...
            <div className="grid grid-cols-5 gap-3">
              {cats.map((c, i) => (
                <Link key={c.id} href={`/product/${c.slug}`} className="flex flex-col items-center justify-start" title={c.name}>
