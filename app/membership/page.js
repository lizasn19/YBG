  ...
  const refreshPoints = useCallback(async () => {
    ...
    try {
      ...
      if (pbErr) {
        ...
        const res = await fetch("/api/points", { headers: { "X-User-Id": uid } });
  ...
  const refreshExpiry = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setExpiry((_) => ({ nextDate: null, nextAmount: 0, soonAmount: 0, daysLeft: null, loading: false }));
        return;
      }
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between bg-white shadow p-4 sticky top-0 z-10">
            ...
            <div className="leading-tight text-[#D6336C] text-sm">
              ...
              {expiry.loading ? (
                ...
                <p>
                  ...
                  <span className="font-semibold">akan hangus</span>{" "}
                  pada {formatDateID(expiry.nextDate)}{" "}
          ...
          <Link href="/history" className="text-[#D6336C] text-sm font-bold">History Poin</Link>
        </div>
        ...
        <section className="px-4 pt-4">
          <div className="flex items-center justify-between">
            <div className="text-sm">
              ...
              <p className="text-base font-semibold text-[#D6336C]">{t.currentTierName}</p>
            </div>
            {t.nextTierName ? (
              ...
              <span className="text-xs text-emerald-600 font-medium">Top Tier</span>
            )}
          </div>
              ...
              {t.nextTierName ? (
                <>
                  ...
                  <span className="ml-1 text-gray-500">
                    ({Math.max(t.base, 0)} / {t.target} · {t.spanPercent.toFixed(1)}%)
                  </span>
                </>
              ) : (
                t.label
        ...
        <div className="p-4 space-y-3">
          {rewards.map((rw) => (
            <Claim
              key={rw.id}
              reward={rw}
              onClaimed={async () => { await refreshPoints(); await refreshExpiry(); }}
              userId={userId}
              points={points}
            />
          ))}
