...
// helper untuk membuat verifier reCAPTCHA (invisible)
export function buildRecaptchaVerifier(containerId = "recaptcha-container") {
  const auth = getFirebaseAuth();
  // kalau sudah ada instance sebelumnya, hapus
...
export async function sendOtp(phoneE164) {
  ...
  window.confirmationResult = confirmation; // simpan untuk verifikasi
  return true;
}
...
export async function verifyOtp(code) {
  ...
  // opsional: signOut dari Firebase (Supabase tetap jadi auth utama)
  await signOut(getFirebaseAuth());
  return cred.user; // mengandung phoneNumber terverifikasi
}